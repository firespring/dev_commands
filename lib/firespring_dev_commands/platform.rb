module Dev
  class Common
    class Platform
      ALLOWED_ARCHITECTURES = %w[arm64 amd64]
      ...
      def architecture
        ...
        if docker_architecture.empty?
          ...
          if docker_architecture.start_with?('linux/')
            architecture_name = docker_architecture.split('/')[1]

            if ALLOWED_ARCHITECTURES.include?(architecture_name)
              docker_architecture
            else
              raise "Invalid DOCKER_ARCHITECTURE: #{architecture_name}. Allowed architectures are #{ALLOWED_ARCHITECTURES.join(', ')}"
            end
          else
            raise "Missing 'linux/' prefix in DOCKER_ARCHITECTURE: #{docker_architecture}"
          end
        end
      end
    end
