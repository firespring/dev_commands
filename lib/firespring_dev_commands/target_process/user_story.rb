  ...
  class TargetProcess
    class UserStory
      ...
      def initialize(data)
        ...
        #@project = Project.new(data['Project'])
        #@owner = User.new(data['User'])
        #@creator = User.new(data['Creator'])
        #@release = Release.new(data['Release'])
        #@team = Team.new(data['Team'])
        @start_date = parse_dot_net_date(data['StartDate'])
        @end_date = parse_dot_net_date(data['EndDate'])
        @create_date = parse_dot_net_date(data['CreateDate'])
        @modify_date = parse_dot_net_date(data['ModifyDate'])
        @tags = data['Tags']
        @effort = data['Effort']
        @time_spent = data['TimeSpent']
        @last_state_change_date = parse_dot_net_date(data['LastStateChangeDate'])
        @original_data = original_data
      end

      def parse_dot_net_time(string)
        Time.at(string.slice(6, 10).to_i)
      end

      def parse_dot_net_date(string)
...
=begin
 ...
 TargetProcess::sbf_projects)
        filter_by_states(query,
 ['In Progress'])
        filter_by_missing_tests(query)

        TargetProcess::get_helper('/UserStories',
 query).length
      end
      ...
      def in_progress(team_id)
        story_info(team_id, ['In Progress'])
      end

      def in_testing(team_id)
        story_info(team_id, ['In Testing'])
      end

      def returned(team_id)
        story_info(team_id, ['Returned'])
      end

      def story_info(team_id, states)
